...
export default function DeploymentDetail({ deploymentId }: Props) {
  ...
  if (error)
    return <div className="err err-banner">Failed to load: {error}</div>;
  if (!data)
    return (
      <div className="empty">
  ...
  async function doRollback() {
    if (
      !window.confirm(
        `Roll back ${data!.clientName} (${data!.domain})? This will stop the container and remove the Caddy route.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setRollbackErr((err as Error).message);
    } finally {
      setRolling(false);
    }
  }
    ...
    <div>
      <div className="detail-head">
        ...
        <div className="detail-status">
          ...
          <span
            className={`dot ${connected ? "" : "dot-off"}`}
      ...
      <div className="detail-meta">
        <div>
          ...
          <span className="val">{data.image}</span>
        </div>
        <div>
          <span className="lbl">Container</span>
          <span className="val">{data.containerName || "—"}</span>
        </div>
        <div>
          <span className="lbl">Container ID</span>
          <span className="val">
            {data.containerId ? data.containerId.slice(0, 12) : "—"}
          </span>
        </div>
        <div>
          <span className="lbl">Host port</span>
          <span className="val">{data.hostPort || "—"}</span>
        </div>
        <div>
          <span className="lbl">Lambda req</span>
          <span className="val">{data.lambdaRequestId || "—"}</span>
        </div>
        <div>
          <span className="lbl">Teardown req</span>
          <span className="val">{data.teardownLambdaRequestId || "—"}</span>
        </div>
      </div>
        ...
        <button
          ...
          title={
            canRollback
              ...
              : "Rollback only available after Completed/Failed"
          }
        >
